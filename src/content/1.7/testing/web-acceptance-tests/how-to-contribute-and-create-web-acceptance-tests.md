...
### Classes (Pages)
#### Name
The name of the class must be simple and linked to the page in PrestaShop. You should be able to find the corresponding page in the application using the folder hierarchy AND the class name alone.
The name of the file is a little different: the main page is always named `index.js`.
Examples: 
...
#### inheritance
We heavily use inheritance in the pages to make sure all our generic methods are available from everywhere and you don’t need to instantiate separate objects. Using inheritance in final classes makes it possible to use the generic and abstracted methods from parent classes to create some more specific methods to use in the child pages.

There is a main mother class, called `CommonPage.js`, located at the root of the `pages` folder.
We have another level of heritage in each root folder (`BO` and `FO`) through the classes `BOmainPage.js` and `FOMainPage.js`. Of course, `CommonPage` is not meant to be instantiated (it’s kind of an abstract class) !
...
#### Methods
Methods inside a page class must be properly named about their primary function. Remember to KISS (Keep it stupid simple : https://en.wikipedia.org/wiki/KISS_principle) ! You should be able to understand what a method does just by looking at its name.
...
##### Helper file    
The helper is used to centralize browser and tab functions called in all tests. This approach has one goal: to have the same browser’s configuration everywhere and of course to facilitate maintenance.
...
`setDownloadBehavior`, employed only for tests that contain files downloading
...
For that, we first created a base context for each and every test file, and after that we make a call to the function `addContextItem` with the unique value inside the test file. 

Example : 
```js
// For test : puppeteer/campaigns/functional/BO/04_customers/01_customers/07_helpCard.js
...
Deleting files (invoices, images) and artefacts
