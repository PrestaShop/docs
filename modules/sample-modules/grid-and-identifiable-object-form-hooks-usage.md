...
This hook, through `$params` array, received `GridDefinition` that defines how the grid is rendered. See [Grid definition]({{< relref "/9/development/components/grid/#grid-definition" >}}) for more information.  
...
`ToggleColumn` - used to display booleans, it will display an icon instead of the value. If user clicks on it, this triggers a toggle of the boolean value. More information about this column and all available parameters can be found [here]({{< relref "/9/development/components/grid/columns-reference/toggle" >}}).  
...
If you only want to display data then this step can be skipped. E.g you are creating `DataColumn`. See [Column references]({{< relref "/9/development/components/grid/columns-reference/" >}}) for full list of grid columns available.
...
about it can be found [here]({{< relref "/9/development/components/form/types-reference/" >}}).
